...
where
      vfq.season = f1_logik.get_cur_f1_season
      ...
      where
         ...
         and vfs.race = (
            select
               max(race)
            from
               f1_access.v_f1_driverstandings
            where
               season = f1_logik.get_cur_f1_season
         )
      group by
         vfs.season
        ,vfd.givenname
...
where
   season = f1_logik.get_cur_f1_season
order by
   number_of_getting_in_points desc
  ,points desc;
...
order by
   f1_logik.to_millis(median_laptime) asc;